...
void TableMutatorSync::handle_exceptions() {
  try {
    throw;
  }
...
TableMutatorSync::TableMutatorSync(PropertiesPtr &props, Comm *comm, Table *table,
    RangeLocatorPtr &range_locator,
    uint32_t timeout_ms, uint32_t flags)
  : m_callback(this), m_last_error(Error::OK), m_last_op(0), m_unflushed_updates(false) {
...
TableMutatorSync::~TableMutatorSync() {
...
void
TableMutatorSync::set(const KeySpec &key, const void *value, uint32_t value_len) {
...
void
TableMutatorSync::set_cells(Cells::const_iterator it, Cells::const_iterator end) {
...
void TableMutatorSync::set_delete(const KeySpec &key) {
...
void TableMutatorSync::auto_flush() {
  try {
    while(m_mutator->has_outstanding()) {
      m_queue->wait_for_buffer();
...
bool TableMutatorSync::retry(uint32_t timeout_ms) {
...
void TableMutatorSync::retry_flush() {
...
std::ostream &
TableMutatorSync::show_failed(const Exception &e, std::ostream &out) {
...
void TableMutatorSync::update_ok() {
  if (m_failed_cells.size() > 0) {
...
void TableMutatorSync::update_error(int error, FailedMutations &failures) {
