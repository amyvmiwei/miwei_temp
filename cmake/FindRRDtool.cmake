...
find_library(XRENDER_LIBRARY NAMES Xrender NO_DEFAULT_PATH PATHS
    ${HT_DEPENDENCY_LIB_DIR}
    /opt/local/lib
    /usr/local/lib
    /usr/lib/i386-linux-gnu
...
find_library(X11_LIBRARY NAMES X11 NO_DEFAULT_PATH PATHS
    ${HT_DEPENDENCY_LIB_DIR}
    /opt/local/lib
    /usr/local/lib
    /usr/lib/i386-linux-gnu
...
find_library(XAU_LIBRARY NAMES Xau NO_DEFAULT_PATH PATHS
    ${HT_DEPENDENCY_LIB_DIR}
    /opt/local/lib
    /usr/local/lib
    /usr/lib/i386-linux-gnu
...
find_library(XCB_LIBRARY NAMES xcb NO_DEFAULT_PATH PATHS
    ${HT_DEPENDENCY_LIB_DIR}
    /opt/local/lib
    /usr/local/lib
    /usr/lib/i386-linux-gnu
...
find_library(XDMCP_LIBRARY NAMES Xdmcp NO_DEFAULT_PATH PATHS
    ${HT_DEPENDENCY_LIB_DIR}
    /opt/local/lib
    /usr/local/lib
    /usr/lib/i386-linux-gnu
...
#message(STATUS "${RRD_LIBRARY} ${RRD_INCLUDE_DIR} ${FREETYPE_LIBRARY} ${PNG_LIBRARY} ${PANGOCAIRO_LIBRARY} ${PANGO_LIBRARY} ${PANGOFT2_LIBRARY} ${CAIRO_LIBRARY} ${PIXMAN_LIBRARY} ${GOBJECT_LIBRARY} ${GMODULE_LIBRARY} ${GTHREAD_LIBRARY} ${GLIB_LIBRARY} ${ART_LGPL_2_LIBRARY} ${XRENDER_LIBRARY}")
  
  if (RRD_LIBRARY AND RRD_INCLUDE_DIR AND FREETYPE_LIBRARY AND PNG_LIBRARY AND PANGOCAIRO_LIBRARY AND PANGO_LIBRARY AND PANGOFT2_LIBRARY AND CAIRO_LIBRARY AND PIXMAN_LIBRARY AND GOBJECT_LIBRARY AND GMODULE_LIBRARY AND GTHREAD_LIBRARY AND GLIB_LIBRARY AND ART_LGPL_2_LIBRARY AND XRENDER_LIBRARY)
    set(RRD_LIBRARIES ${RRD_LIBRARY} ${FREETYPE_LIBRARY} ${PNG_LIBRARY} ${PANGOCAIRO_LIBRARY} ${PANGO_LIBRARY} ${PANGOFT2_LIBRARY} ${CAIRO_LIBRARY} ${PIXMAN_LIBRARY} ${GOBJECT_LIBRARY} ${GMODULE_LIBRARY} ${GTHREAD_LIBRARY} ${GLIB_LIBRARY} ${ART_LGPL_2_LIBRARY} ${XRENDER_LIBRARY} ${X11_LIBRARY} ${XAU_LIBRARY} ${XCB_LIBRARY} ${XDMCP_LIBRARY})
    set(RRD_FOUND TRUE)
